...
uint8_t SPIx_WriteRead(uint8_t Byte)
{
  ...
  if(HAL_SPI_TransmitReceive(&hspi1,(uint8_t*) &Byte,(uint8_t*) &receivedbyte,1,0x1000)!=HAL_OK) Error();
  return receivedbyte;
}
...
uint8_t SD_Write_Block (uint8_t *buff, uint32_t lba, uint16_t Size)
{
  uint8_t result;
  uint16_t cnt;
  ...
  for (cnt=0;cnt<Size;cnt++) SPI_SendByte(buff[cnt]); //Данные
...
uint8_t SD_Read_Block(uint8_t *buff, uint32_t lba, uint16_t Size)
{
	uint8_t result;
  uint16_t cnt;
  ...
  //for (cnt=0;cnt<Size;cnt++) buff[cnt]=SPI_ReceiveByte(); //получаем байты блока из шины в буфер
	for (cnt=0;cnt<512;cnt++) buff[cnt<=Size?cnt:Size]=SPI_ReceiveByte(); //получаем байты блока из шины в буфер
...
uint8_t sd_ini(void)
{
  ...
  sdinfo.type = 0;
	...
	//hspi1.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_64;//SPI_BAUDRATEPRESCALER_128; //156.25 kbbs
	//HAL_SPI_Init(&hspi1);
	SS_SD_DESELECT();
	for(i=0;i<10;i++) SPI_Release();//80 импульсов (не менее 74) Даташит стр 91
